...
For a new repository, open **Settings → Pages** and select **GitHub Actions** as the source. This setting was confirmed for this repository on 9 September 2026. The intended project URL is `https://yossefseit.github.io/egypt-salary-calculator/`; the reviewed production bundle still needs publication through `main`. The deployment job has only the `pages: write` and `id-token: write` permissions required by GitHub Pages; the build job has read-only access to repository contents and Pages metadata.
